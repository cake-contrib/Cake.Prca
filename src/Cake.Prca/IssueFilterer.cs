namespace Cake.Prca
{
    ...
    /// <summary>
    /// Class for filtering code analysis issues.
    /// </summary>
    internal class IssueFilterer
    {
            ...
            IDictionary<ICodeAnalysisIssue, IEnumerable<IPrcaDiscussionComment>> issueComments)
        {
            ...
            // ReSharper disable once PossibleMultipleEnumeration
            var codeAnalysisIssues = issues as IList<ICodeAnalysisIssue> ?? issues.ToList();

            var result = this.FilterIssuesByPath(codeAnalysisIssues);
        ...
        private IList<ICodeAnalysisIssue> FilterIssuesByPath(IList<ICodeAnalysisIssue> issues)
        {
            if (!issues.Any())
            {
                return issues;
        ...
        private IList<ICodeAnalysisIssue> FilterIssuesByNumber(IList<ICodeAnalysisIssue> issues)
        {
            if (!issues.Any())
            {
                return issues;
